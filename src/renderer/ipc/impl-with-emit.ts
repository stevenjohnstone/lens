...
import emitInjectable from "./emit-to-main.injectable";
import type { Injectable, InjectionToken } from "@ogre-tools/injectable";

export function implWithEmit<Token extends Channel<any[], void>>(channelToken: Token) {
  return channelToken.getInjectable((di, channel) => {
    const emit = di.inject(emitInjectable);

    return (...args: any[]) => emit(channel, ...args);
