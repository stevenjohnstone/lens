...
const broadcastMessageInjectable = implWithOn(broadcastMessageInjectionToken, (di) => (
  broadcastMessage({
    ipcMain: di.inject(ipcMainInjectable),
    getAllWebContents: di.inject(getAllWebContentsInjectable),
    clusterFrames: di.inject(clusterFramesInjectable),
    logger: di.inject(ipcLoggerInjectable),
  })
));
